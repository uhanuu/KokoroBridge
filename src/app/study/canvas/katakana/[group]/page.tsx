...
export default function KatakanaGroupPage() {
  ...
  const strokeDataMap = useMemo(() => {
    ...
    katakanaStrokeData.forEach(data => {
  ...
  const handleNextGroup = useCallback(() => {
    ...
    const currentGroupIndex = katakanaGroups.findIndex(group => group.id === currentGroup.id);
    const nextGroup = katakanaGroups[currentGroupIndex + 1];
    if (nextGroup) {
  ...
  // 다음 그룹 존재 여부 확인
  const currentGroupIndex = katakanaGroups.findIndex(group => group.id === currentGroup.id);
  const hasNextGroup = currentGroupIndex < katakanaGroups.length - 1;
